...
def main(invd, noOfExams, takee, i):
    ...
    comb_dict = {new_list[i]: new_list[i + 1]
    ...
    noCustomTeachers = 0
    if takee != 0:
        for _ in range(len(takee)):
            work_days, *teacherName = takee[_].split()
            print(f"{teacherName[0]} {teacherName[1]}".title())
            indexCustomTeacher.append(
                comb_dict[f"{teacherName[0]} {teacherName[1]}".title()])
            customTeacherDays.append(int(work_days))
        print(indexCustomTeacher)

        if len(takee) != 0:
            noCustomTeachers = len(takee)
    ...
    for _ in range(days):
        ...
        arr3 = []
    # arr5 = []
    fileName = 'invigilators' if i == 1 else 'test'
...
def passer(take, spb, ratio, takee):
    ...
    for _ in range(noOfExams):
        ...
        no_blocks = math.ceil(int(take[_])/int(spb))
        no_reliever = math.ceil(no_blocks/5)
        no_extras = math.ceil(no_blocks/int(ratio))

        no_inv = no_blocks + no_reliever + no_extras
        invd.append(no_inv)
    main(invd, noOfExams, takee, i=1)


@eel.expose
def no_stu(take, spb, ratio, takee):
    passer(take, spb, ratio, takee)
    print(takee)


@eel.expose
def tester():
    invd = [34, 39, 13, 38, 8, 30, 31, 17, 37, 32,
            26, 25, 29, 23, 34, 22, 32, 25, 17, 14]
    main(invd, 20, 0, i=0)


eel.start('home.html', size=(900, 600))
